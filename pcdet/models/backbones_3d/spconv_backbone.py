...
class SparseBasicBlock(spconv.SparseModule):
    ...
    def __init__(self, inplanes, planes, stride=1, norm_fn=None, downsample=None, indice_key=None, act_fn=nn.ReLU):
        ...
        #     inplanes, planes, kernel_size=1, stride=1, padding=0, bias=False, indice_key=indice_key)
    ...
    def forward(self, x):
        ...
        # if self.downsample is not None:
        #     # if identity.features.shape != out.features.shape:
        #     identity = self.downsample(x)
        #     if self.bn3 is not None:
        #         identity = replace_feature(identity, self.bn3(identity.features))
...
class VoxelResBackBone8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        if len(layer_nums) == 6:
        ...
        elif len(layer_nums) == 13:
            for k in range(2 - 1):
                conv1_list.append(
                    SparseBasicBlock(num_filters[1], num_filters[2], norm_fn=norm_fn, act_fn=act_fn, indice_key='res1'))
            self.conv1 = spconv.SparseSequential(*conv1_list)

            # conv2
            conv2_list = [
                # [1600, 1408, 41] <- [800, 704, 21]
                block(num_filters[2], num_filters[3], 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv2',
                      conv_type='spconv'),
            ]
            for k in range(3 - 1):
                conv2_list.append(
                    SparseBasicBlock(num_filters[3+k], num_filters[4+k], norm_fn=norm_fn, act_fn=act_fn, indice_key='res2'))
            self.conv2 = spconv.SparseSequential(*conv2_list)

            # conv3
            conv3_list = [
                # [800, 704, 21] <- [400, 352, 11]
                block(num_filters[5], num_filters[6], 3, norm_fn=norm_fn, stride=2, padding=1, indice_key='spconv3',
                      conv_type='spconv'),
            ]
            for k in range(3 - 1):
                conv3_list.append(SparseBasicBlock(
                    num_filters[6+k], num_filters[7+k], norm_fn=norm_fn, act_fn=act_fn, indice_key='res3'
                ))
            self.conv3 = spconv.SparseSequential(*conv3_list)
            ...
            conv4_list = [
                # [400, 352, 11] <- [200, 176, 5]
                block(num_filters[8], num_filters[9], 3, norm_fn=norm_fn, stride=2, padding=(0, 1, 1),
                      indice_key='spconv4', conv_type='spconv'),
            ]
            for k in range(3 - 1):
                conv4_list.append(SparseBasicBlock(
                    num_filters[9+k], num_filters[10+k], norm_fn=norm_fn, act_fn=act_fn, indice_key='res4'
                ))
            self.conv4 = spconv.SparseSequential(*conv4_list)
            ...
            self.conv_out = spconv.SparseSequential(
                # [200, 150, 5] -> [200, 150, 2]
                spconv.SparseConv3d(num_filters[11], num_filters[12], (3, 1, 1), stride=(2, 1, 1), padding=last_pad,
                                    bias=False, indice_key='spconv_down2'),
                norm_fn(num_filters[12])
            )

            self.final_act = act_fn()

            self.num_point_features = num_filters[12]
            self.backbone_channels = {
                'x_conv1': num_filters[1],
                'x_conv2': num_filters[3],
                'x_conv3': num_filters[6],
                'x_conv4': num_filters[9]
            }
