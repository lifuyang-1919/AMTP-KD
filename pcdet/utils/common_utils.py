...
# ----------------------RKD_distance-------------------------------------------
def pair_distance_gpu1(e, squared=True, eps=1e-12):
    '''
    ...
    prod = 2 * torch.mm(e, e.transpose(1, 0))
    ...
    res = res.clone()
    res[range(len(e)), range(len(e))] = 0  # 主对角线置0
    return res

def pair_angle_gpu(teacher):
    td = (teacher.unsqueeze(0) - teacher.unsqueeze(1))
    norm_td = torch.nn.functional.normalize(td, p=2, dim=2)
    t_angle = torch.bmm(norm_td, norm_td.transpose(1, 2)).view(teacher.shape[0], -1)
    return t_angle
